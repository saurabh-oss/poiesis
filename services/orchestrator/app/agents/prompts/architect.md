...
Every `web-app` runs the same standard service topology, generated by the platform:
a **gateway** (nginx: the UI and /api routing), an **api** service (the story endpoints
plus a generic data API over every table), a **data** service (the generic data API
only, which the gateway falls back to when the api is down), and **Postgres**. Each
service has a health check and restarts on failure, and a story module that fails to
load is isolated instead of taking the api down. Do not design another topology, and
do not list these as components or as work. List only the components that carry this
product's own behaviour, as modules of the api service.
