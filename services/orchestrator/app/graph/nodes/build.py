    ...
    *test* was the thing that was wrong. In a verified run the Tester read
    ...
    try:
        revision = await remember(run_id, f"reconcile:{sid}:r{rnd}", lambda: TESTER.json(
            ...
            "implementation (assert it is present and names the key terms instead), "
            "asserts an exact count of a ranked, filtered or scored result where the criterion "
            "only fixes a maximum or a threshold (\"top 3\" means at most 3: assert "
            "`1 <= len(body) <= 3`, and that the best match comes first), or "
